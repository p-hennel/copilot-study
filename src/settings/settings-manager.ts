...
export const defaultSettingsSchema = z.object({
  paths: z.object({
    ...
    database: z.string().default(`file://${join(PathResolver.getDataRoot(), "config", "main.db")}`),
  ...
  hashing: z.object({
    algorithm: z.enum([
      "sha256", "sha512", "blake2b512", "md5", "sha1", 
  ...
  auth: z.object({
    initCode: z.string().default(process.env.INIT_CODE || "aVNEpnVwsutCH5sq4HGuQCyoFRFh7ifneoiZogrpV2EoLRsc"),
 ...
 */
export function createSettingsManager<T>(schema: z.ZodSchema<T>, options: Omit<SettingsManagerOptions<T>, 'schema'> = {}) {
