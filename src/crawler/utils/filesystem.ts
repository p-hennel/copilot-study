...
export async function appendToJsonlFile(filePath: string, data: any | any[]): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to append to ${filePath}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export async function readJsonlFile<T = any>(filePath: string): Promise<T[]> {
  try {
    ...
    throw new Error(
      `Failed to read JSONL file ${filePath}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
