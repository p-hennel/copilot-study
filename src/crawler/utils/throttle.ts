...
export function createThrottledRequest<T>(options: ThrottleOptions) {
  const {
    requestsPerSecond,
    baseRetryDelayMs,
    backoffFactor,
  ...
  const throttleFn = async <T>(fn: () => Promise<T>): Promise<T> => {
    ...
    try {
      ...
      const baseBackoff = baseRetryDelayMs * Math.pow(backoffFactor, consecutiveErrors - 1);

      // Add jitter to prevent thundering herd
      const jitter = 1 + Math.random() * jitterFactor * 2 - jitterFactor;
      const backoffTime = Math.min(baseBackoff * jitter, maxBackoffMs);
