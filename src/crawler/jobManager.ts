...
import type { ProjectSchema, GroupSchema } from "@gitbeaker/rest"; // Added imports for Gitbeaker types
...
export class JobManager {
  ...
  private sendCompletionUpdate(job: Job) {
    ...
    if (job.status !== "completed" && job.status !== "failed") {
      console.warn(
        `sendCompletionUpdate called for job ${job.id} with unexpected status: ${job.status}`
      );
      return;
    }
  ...
  private pause() {
    if (this.state === "running") {
      ...
      if (this.activeJob) {
        console.log(
          `Pause requested for active job: ${this.activeJob.id}. Will pause before next API call.`
        );
  ...
  private async executeJob(job: Job, client: GitlabClient): Promise<void> {
    console.log(
      `Executing job ${job.id}: Fetching ${job.dataTypes.join(", ")} for ${job.targetPath}`
    );
    ...
    for (const dataType of job.dataTypes) {
      ...
      while (hasNextPage) {
        ...
        if (this.state === "paused") {
          ...
          console.log(
            `Job ${job.id} paused (global state) during pagination of ${dataType}. Saving progress.`
          );
            ...
            try {
              ...
              if (dataType === "groupProjects") {
                ...
                for (const project of result.data as ProjectSchema[]) {
                  ...
                  if (discoveredPath) {
                    console.log(`  - Discovered project: ${discoveredPath}. Spawning jobs...`);
                ...
                for (const group of result.data as GroupSchema[]) {
                  ...
                  if (discoveredPath) {
                    console.log(`  - Discovered sub-group: ${discoveredPath}. Spawning jobs...`);
              ...
              console.error(`  - Error during job spawning logic for ${dataType}:`, spawnError);
    ...
    console.log(
      `Finished processing all data types for job ${job.id}. Final status: ${job.status}`
    );
