...
async function main() {
  ...
  client.on('config', (_originId, config?: NestedOmit<NestedOmit<NestedOmit<Omit<CrawlerConfig, "hooks">, "includeResources.projectFilterFn">, "includeResources.groupFilterFn">, "auth.tokenRefreshCallback">) => {
    config = {
      ...config,
      hooks: {
  ...
  let heartbeat = 5000
  try { heartbeat = parseInt(values.heartbeat) }
  catch { ; }
  if (heartbeat > 0) {
    setInterval(() => {
