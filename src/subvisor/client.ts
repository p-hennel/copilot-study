...
export class SupervisorClient extends EventEmitter {
  ...
  private pruneMessageQueue(): void {
    ...
    this.logger.warn(`Message queue exceeded limit (${this.messageQueue.length}), pruning oldest messages`, {
    ...
    this.messageQueue.sort((a, b) => {
      const getPriority = (type: MessageType) => {
        switch (type) {
          case MessageType.COMMAND: return 0;
          case MessageType.STATE_CHANGE: return 1;
          case MessageType.HEARTBEAT: return 3;
          case MessageType.MESSAGE: return 2;
          default: return 4;
        }
      };
  ...
  private handleMessage(data: Buffer): void {
    try {
      ...
      if (message.destination !== this.id) {
        this.logger.warn(`Received message intended for ${message.destination}, but our ID is ${this.id}`, {
      ...
      switch (message.type) {
        ...
        case MessageType.STATE_CHANGE:
          ...
          this.emit('stateChange', message.origin, message.payload.newState, message.payload.oldState);
          break;
