...
export class ManagedProcess extends EventEmitter {
  ...
  public start(): void {
    ...
    try {
      ...
      this.process.exited.then((code: any) => {
  ...
  public stop(): Promise<void> {
    return new Promise((resolve) => {
      if (!this.process || this.state === ProcessState.STOPPED || this.state === ProcessState.STOPPING) {
        resolve();
        return;
      }
  ...
  private calculateBackoff(): number {
    ...
    const exponentialDelay = Math.min(
      baseDelay * Math.pow(2, this.restartCount), 
      maxDelay
    );
  ...
  private checkProcessHealth(): void {
    ...
    if (cpuUsage > 95 && this.lastCpuUsage > 95) {
      ...
      if (duration > 30) { // 30 seconds of high CPU
        this.logger.warn(`Process ${this.id} might be deadlocked (CPU usage > 95% for ${duration}s)`, {
          cpu: cpuUsage,
          duration,
