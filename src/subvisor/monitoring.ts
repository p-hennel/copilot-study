...
export class MonitoringServer {
  ...
  private handleSettingsChange(event: SettingsChangeEvent): void {
    ...
    if (oldConfig.monitoringPort !== newConfig.monitoringPort) {
      this.logger.info(
        `Monitoring port changed from ${oldConfig.monitoringPort} to ${newConfig.monitoringPort}`
      );
  ...
  private handleMetricsRequest(): Response {
    ...
    for (const [id, process] of this.supervisor["processes"].entries()) {
      metrics.push(
        `supervisor_process_restart_count{id="${id}"} ${process.getRestartCount() || 0}`
      );
    }
  ...
  private handleHealthRequest(): Response {
    ...
    const failedCount = Array.from(processes.values()).filter(
      (p) => p.getState() === ProcessState.FAILED
    ).length;
