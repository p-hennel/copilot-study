...
export async function boot() {
  ...
  client.on('needsConfig', () => {
    client.emit('config', {
      ...
      auth: {
        clientId: dev ? AppSettings().auth.providers.gitlabCloud.clientId : AppSettings().auth.providers.gitlab.clientId,
        clientSecret: dev ? AppSettings().auth.providers.gitlabCloud.clientSecret : AppSettings().auth.providers.gitlab.clientSecret,
      },
  ...
  client.on('jobFailed', (event: { job: Job, event: JobFailedEvent }) => {
    try {
      ...
      logger.error("job {id} failed: {event}", {id: job.id, job, msg: event.event.error, event: event.event})
    }
  })
